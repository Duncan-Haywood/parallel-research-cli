"""Agent definitions for the EssayForge research synthesis system."""
...
class AgentType(Enum):
    """Basic agent types for research."""
...
@dataclass
class Agent:
    """Basic research agent configuration."""
    type: AgentType
    description: str
    prompt_template: str
    ...
    def generate_prompt(self, topic: str) -> str:
        ...
        return self.prompt_template.format(topic)


def create_agents(intensity: int) -> List[Agent]:
    """Create a list of agents based on the intensity level."""
    all_agents = [
        Agent(
            type=AgentType.FACT_GATHERER,
            description="Gathers core facts and verifiable information",
            prompt_template="""You are a fact-gathering research agent. Your task is to research "{}" and provide:
...
Focus on accuracy and cite all sources. Output in markdown with proper citations."""
        ...
        Agent(
            ...
            prompt_template="""You are a current affairs research agent. Research the current state of "{}" including:
...
Provide timely, up-to-date information with sources. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are an expert opinion researcher. For the topic "{}", gather:
...
Quote experts directly when possible and cite sources. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are a historical research agent. Provide historical context for "{}" including:
...
Focus on how the past informs the present. Output in markdown with citations."""
        ...
        Agent(
            ...
            prompt_template="""You are a critical analysis agent. For "{}", research:
...
Be balanced and fair in presenting different sides. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are a future trends analyst. For "{}", research and analyze:
...
Base projections on current data and expert analysis. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are a case study researcher. For "{}", provide:
...
Focus on concrete examples with details. Output in markdown with sources."""
        ...
        Agent(
            ...
            prompt_template="""You are a data analysis agent. For "{}", provide:
...
Focus on numbers and measurable outcomes. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are a theoretical research agent. For "{}", explore:
...
Connect theory to practical understanding. Output in markdown."""
        ...
        Agent(
            ...
            prompt_template="""You are a practical applications researcher. For "{}", detail:
